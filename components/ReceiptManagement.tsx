...
export const ReceiptManagement: React.FC<ReceiptManagementProps> = ({
      ...
      <>
        ...
        {selectedHeader?.status === 'Gebucht' && (
            <button
                ...
                className={`px-3 py-1.5 rounded-lg border text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap ${
                    isDark 
  ...
  return (
    <div className="h-full flex flex-col animate-in slide-in-from-right-8 duration-300 pb-20 lg:pb-0">
      ...
      {/* PANE A: HEADER & STATUS CONTROL (Adaptive) */}
      <div className={`mb-6 rounded-2xl border flex flex-col transition-all duration-300 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} ${activeTab === 'tickets' ? 'p-4' : 'p-4 md:p-6'}`}>
        ...
        {activeTab === 'items' ? (
            ...
            // --- COMPACT HEADER MODE (Active Tab: Tickets) ---
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                    <button onClick={handleBack} className="p-2 -ml-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500">
                        <ArrowLeft size={20} />
                    </button>
                    <div className="h-8 w-px bg-slate-500/20 hidden sm:block"></div>
                    <div>
                        <div className="flex items-center gap-3">
                            <span className={`text-lg font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
                                PO: {selectedHeader.bestellNr || '—'}
                            </span>
                        ...
                        <div className="text-xs opacity-50 font-medium">
                            LS: {selectedHeader.lieferscheinNr} • {relatedTickets.length} Fälle
                        </div>
                    </div>
                </div>
                
                <div className="flex items-center gap-2 pt-2 sm:pt-0 border-t sm:border-0 border-slate-500/10 justify-end w-full sm:w-auto">
                    {/* Compact actions for mobile/desktop */}
                    <div className="sm:hidden flex mr-auto">
                         {renderReceiptBadges(selectedHeader, linkedPO, linkedMaster, relatedTickets)}
                    </div>
                    {renderActions()}
                </div>
      ...
      {/* TICKET TAB CONTENT */}
      {activeTab === 'tickets' && selectedBatchId && (
        <TicketSystem 
